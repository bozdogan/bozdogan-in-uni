#!/usr/bin/env python3
# bozdogan Game Engine
"""
pixel based game engine

designed to be compatible with javidx9's olcPixelGameEngine
so I can implement his projects
"""

import time

# Mute pygame's welcome message
...
class BozPixelGE:
    ...
    def start(self):
        if self.screen is None:
            print("Screen surface does not exist. (Probably ´start´ is called before ´construct´.)")
            return False
        ...
        # Game loop
        self.clock[0], self.clock[1] = self.clock[1], time.clock()  # clock tick
    ...
    def on_engine_create(self):
        """This method is called once, after initializing the display. Load your surfaces here.
    ...
    def set_event_handler(self, event_type, handler):
        """Assign or change the handler for a particular event. ´handler´ takes 1 argument, which is event."""
    ...
    def draw_rect(self, pos, size, color, bd_width=0):
        """Draw a rectangle on canvas. If border width isn't specified, it'll fill the shape."""
        pygame.draw.rect(self.canvas, color, (*pos, *size), bd_width)

    def draw_circ(self, pos, color, radius, bd_width=0):
        """Draw a circle on canvas. If border width isn't specified, it'll fill the shape."""
