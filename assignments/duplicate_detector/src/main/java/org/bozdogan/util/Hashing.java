...
public class Hashing{
    ...
    /** Creates a SHA1 summary of given file.
     * @param inputFile path of the file */
    public static byte[] sha1FromFile(Path inputFile) throws IOException{ // I use this to get a 128bit long key.
        try{
            MessageDigest sha1 = MessageDigest.getInstance("SHA-1");
            InputStream is = Files.newInputStream(inputFile);
            byte[] buffer = new byte[8192]; // this buffer size is for sha-1

            for(int len; (len=is.read(buffer)) != -1; )
                if (len > 0) sha1.update(buffer, 0, len);

            return sha1.digest();
        } catch(NoSuchAlgorithmException e){
            throw new RuntimeException("INTERNAL ERROR: HASHING ALGORITHM NOT FOUND");
        }
    }
